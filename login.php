<?php
	...
	switch($cmd)
	{
		...
		case "login":
			$info["table"]     = "users";
			$info["fields"]   = array("*");
			$info["where"]    = " 1=1 AND email  LIKE  '".clean($_REQUEST["email"])."' AND password  LIKE  '".clean($_REQUEST["password"])."' AND user_type='agent'";			
			$res  = $db->select($info);
			if(count($res)>0)
			{
		...
		case "forget_pass":
		      $info["table"]     = "users";
				...
				if(count($res)>0)
				{
					...
					$subject = "Recovery Password from Travel";
					...
					//send email
						...
						$headers .= 'From: Travel <info@Travel.com>' . "\r\n";
